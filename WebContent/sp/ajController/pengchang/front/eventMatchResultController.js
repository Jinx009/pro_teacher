zaApp.controller('eventMatchResultController',function($scope, $rootScope, $http, $sce, wxJSSDKService, QueryParamService, sessionService){
	...
	$scope.saveNewResult = function(){
		var userData = {
				rivalId : $scope.pageData.currentSelectedRivalId,
				eventId : $scope.pageData.eventId,
				result : $scope.pageData.currentSelectedResult,
				};
		sessionService.postCall(
				$rootScope.urlPath + "/doSaveMatchEventResult.html",
				userData,
				function(data){
					if(data.retCode == 0){
						location.reload();

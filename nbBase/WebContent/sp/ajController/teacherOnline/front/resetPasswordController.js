//var cfApp = angular.module('CFApp', ['ngMd5' // you may also use 'ngMd5' or 'gdi2290.md5' or 'angular-md5'
//                                    ]);

zaApp.controller('resetPasswordController',function($scope, $rootScope, $http, md5, $timeout, sessionService, QueryParamService) {
	...
	$scope.checkInput = function(isCheckForSubmit, checkTarget){
		...
		if( checkTarget == "verifyCode" || checkTarget == "all"){
			...
			if( $scope.verifyCode.length == 4){//每次输入4个的时候，去后台拿一下最新的验证码
		...
		if( $scope.flagPhoneNumberError == 1 ||
			$scope.flagPhoneCodeError == 1 ||
			$scope.flagVerifyCodeError == 1 ||
			$scope.flagPasswordError == 1 ||
			$scope.flagConfirmPasswordError == 1 ){
	...
	$scope.resetPassword = function(){
		...
		var url = $rootScope.urlPath+"/doResetPassword.html";
